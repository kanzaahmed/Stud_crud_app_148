 ...
 */ 
function sync (source, destination, dryRun = false) {   const rsync = new Rsync() 
    .shell('ssh') 
    .set('stats') 
    .flags('avz') 
    .source(source) 
  ...
  rsync.output(     function (data) { 
      ...
      const string = Buffer.from(data).toString()       console.log(string) 
    }, function (data) { 
      // do things like parse error output       console.log(data) 
    } 
  ) 
